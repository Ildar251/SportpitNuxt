...
const stickers = computed(() =>
	product.value?.sticker ? product.value.sticker.split('||') : []
)
...
<template>
	<main>
		<section class="section section-product" v-if="product">
			<div class="container product">
				<div class="product__image">
					<NuxtImg :src="config.public.apiUrl + tastes[activeTasteIndex].image
							? config.public.apiUrl + tastes[activeTasteIndex].image
							: product.image
						" :alt="product.title" height="420" loading="lazy" placeholder=".././images/box.svg" />
				...
				<div class="product__info">
					...
					<div class="product__info-main">
						<div class="product__info-left">
							<h1 class="h1">
								{{ product.title }} {{ tastes[activeTasteIndex].taste }}
							</h1>
							<div class="product__text-block" v-if="product.product_text">
								<div ref="textRef" class="product__text" :class="{
									'product__text--collapsed': !isTextExpanded && isTextLong,
								...
								<button v-if="isTextLong" class="btn btn-more-down"
									...
									<span>{{
										isTextExpanded ? 'Скрыть' : 'Полное описание'
									}}</span>
							...
							<SwiperSlide v-for="(taste, index) in tastes" :key="taste.MIGX_id" class="taste__item"
								...
								<div class="product__taste product__info-block" v-if="tastes.length">
									...
									<Swiper :modules="[Pagination, Autoplay]" :breakpoints="{
										...
										<SwiperSlide v-for="(taste, index) in tastes" :key="taste.MIGX_id"
											class="taste__item"
											:class="{ 'taste__item--active': index === activeTasteIndex }"
											@click="onTasteClick(index)">
											<div class="taste__image">
												<NuxtImg :src="config.public.apiUrl + taste.image" :alt="taste.taste"
													height="120" width="auto" format="webp"
									...
									<div class="dop-info" v-if="!authStore.apiToken">
										...
										<div class="product__price-info">
											...
											<div class="warning">
												...
												<div>
													<span>Чтобы узнать оптовую стоимость, зарегистрируйтесь на
														сайте</span>
								...
								<div class="accordions">
									<UiAccordion v-for="(accordion, index) in accordions" :key="index"
		...
		<div class="minimal" v-else>Минимальная сумма заказа 10 000 ₽</div>
		</div>
		</div>
		</div>
		</section>
