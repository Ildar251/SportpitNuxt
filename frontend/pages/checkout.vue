...
const submitOrder = async () => {
    ...
    const orderData = {
        ...
        paymentMethod: paymentOptionsActive.value,
        deliveryMethod: deliveryOptionsActive.value,
    }

    try {
        const response = await $fetch<{ success: boolean; message?: string; orderId?: string }>(
            'https://test.top-nnov.ru/api/placeOrder',
            {
                method: 'POST',
                body: orderData,
            }
        ...
        if (response.success) {
            ...
            alert('Ошибка оформления заказа: ' + response.message)
        }
    } catch (error) {
        console.error('Ошибка при оформлении заказа:', error)
        alert(`Не удалось оформить заказ. Сервер ответил: ${JSON.stringify(error)}`)
    }
}
            ...
            <div class="container checkout">
                <div class="checkout__item personal-data">
                    ...
                    <div v-if="authStore.user" class="checkout__item-info">
                        <p><strong>Имя:</strong> {{ authStore.user.name || 'Не указано' }}</p>
                        <p><strong>Email:</strong> {{ authStore.user.email || 'Не указано' }}</p>
                        <p><strong>Телефон:</strong> {{ authStore.user.phone || 'Не указано' }}</p>
                    </div>
                </div>
                            ...
                            <div v-else-if="deliveryOptionsActive === 'transport'">
                                <div class="info">
                                    ...
                                    <p>
                                        ...
                                        отправления. Если Вы не получили извещение о посылке из отделения Почты России,
                                        сообщите менеджеру магазина - выясним место нахождения посылки до истечения
                                        срока хранения. Почта России хранит заказы 30 дней. Стоимость доставки
                                        рассчитывается в зависимости от веса товара и удаленности отправления.
                                    </p>
                                </div>
                            </div>
                        </Transition>
